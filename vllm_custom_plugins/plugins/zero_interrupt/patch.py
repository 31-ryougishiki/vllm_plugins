...
def apply():
    ...
    当 ``VLLM_ITS_DEEPSEEK_V4=1`` 时，切换到
    :mod:`zero_interrupt.deepseekv4` 目录下的 DeepSeek-V4 实现；
    否则使用主目录（vllm_plugins_0829 合并实现）。
    """
    if is_deepseek_v4_enabled():
        logger.info(
            "VLLM_ITS_DEEPSEEK_V4=1: applying DeepSeek-V4 patch family"
        )
        from .deepseekv4.patch import apply as deepseek_v4_apply

        return deepseek_v4_apply()
