...
# 支持非对称的ColumnParallelLinearAsymmetric类 继承ColumnParallelLinear
# 修改1：需要从上层获取is_qkv_type num_heads num_kv_heads head_size
# 修改2：需要知道整体切分为多少份，当前卡切分为多少份
class ColumnParallelLinearAsymmetric(ColumnParallelLinear):
    def __init__(
            self,
            input_size: int,
            output_size: int,
            ...
            *,
            ...
            is_qkv_type: bool = False,
            num_heads: int = 0,
            num_kv_heads: int = 0,
            head_size: int = 0,
            tp_asymmetric_shardings: list = None,  # 非对称新增
    ):
        # Divide the weight matrix along the last dimension.
        self.tp_rank = get_tensor_model_parallel_rank() if not disable_tp else 0
        self.tp_size = get_tensor_model_parallel_world_size() if not disable_tp else 1
        self.input_size_per_partition = input_size
        # [lqf] DeepSeek-V4 heterogeneous restart: read
        # tp_asymmetric_shardings from zero_interrupt_config when the caller
        # did not pass it explicitly (e.g. [2,1,1] for tp=3 on DP0).
    ...
    def weight_loader(self, param: Parameter, loaded_weight: torch.Tensor):
        """Asymmetric TP-aware weight_loader for ColumnParallelLinear.

        When asymmetric TP is active (tp_asymmetric_shardings has non-uniform values),
        uses cumulative offsets for calculating start_idx. Otherwise falls back to
        the original symmetric TP logic.
        """
        ...
        # Check if asymmetric TP is active for this layer
        # Must check if shardings are NON-UNIFORM (not just current rank's shard size)
        # e.g., [1,1,2] is non-uniform (sum=4, len=3, each=1.33), [1,1,1] is uniform
        is_asymmetric_active = False
        if self.tp_asymmetric_shardings is not None and len(self.tp_asymmetric_shardings) > 1:
            # Check if shardings are non-uniform (not all equal to 1)
            is_asymmetric_active = any(s != self.tp_asymmetric_shardings[0] for s in self.tp_asymmetric_shardings)
        ...
        # Special case for loading scales off disk
...
# 支持非对称的QKVParallelLinearAsymmetric类 继承ColumnParallelLinearAsymmetric
class QKVParallelLinearAsymmetric(ColumnParallelLinearAsymmetric):
    def __init__(
            self,
            hidden_size: int,
            head_size: int,
            ...
            *,
            ...
            tp_asymmetric_shardings: list = None,  # 非对称增加参数下发
    ):
        ...
        self.num_heads = divide(self.total_num_heads, world_split_size) * split_size
        # end
        if world_split_size >= self.total_num_kv_heads:  # 默认world_split_size和total_num_kv_heads是倍数关系
            if (split_size % divide(world_split_size, self.total_num_kv_heads) and
                    divide(world_split_size, self.total_num_kv_heads) % split_size):
                # TODO: 配置里判断这种情况, 或者之后做适配
                pass
            elif split_size * self.total_num_kv_heads // world_split_size:
                self.num_kv_heads = divide(self.total_num_kv_heads * split_size, world_split_size)
                self.num_kv_head_replicas = 1
            else:
                self.num_kv_heads = 1
                self.num_kv_head_replicas = divide(world_split_size, self.total_num_kv_heads * split_size)
        else:
            self.num_kv_heads = divide(self.total_num_kv_heads, world_split_size) * split_size
            self.num_kv_head_replicas = 1
        ...
        super().__init__(
            ...
            tp_asymmetric_shardings=self.tp_asymmetric_shardings,
        )
...
class RowParallelLinearAsymmetric(RowParallelLinear):
    def __init__(
            self,
            input_size: int,
            output_size: int,
            ...
            *,
            return_bias: bool = True,
            disable_tp: bool = False,
            tp_asymmetric_shardings: list = None,
    ):
    ...
    def forward(self, input_: torch.Tensor) -> torch.Tensor | tuple[torch.Tensor, torch.Tensor]:
        """Forward method with detailed logging for debugging all_reduce."""
        tp_rank = get_tensor_model_parallel_rank()
        tp_size = get_tensor_model_parallel_world_size()

        # Manual matmul to capture pre-all_reduce value
        weight_t = self.weight.t()

        # Call parent forward (includes matmul and all_reduce)
        result = super().forward(input_)

        return result


class MergedColumnParallelLinearAsymmetric(ColumnParallelLinearAsymmetric):
    ...
    def weight_loader(
        ...
        if output_dim is not None:
            ...
            # 计算 output_partition_sizes 的累积偏移作为参考
            local_cumulative = [0]
            for size in self.output_partition_sizes[:-1]:
                local_cumulative.append(local_cumulative[-1] + size)
