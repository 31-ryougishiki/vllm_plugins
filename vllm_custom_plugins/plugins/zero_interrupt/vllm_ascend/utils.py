...
_DIRECT_REGISTER_CUSTOM_OP_PATCHED = False


def _is_op_registered(op_name: str, schema_str: str = "(Tensor x, Tensor weight, Tensor? bias=None) -> Tensor") -> bool:
    try:
        namespace = op_name.split("::")[0] if "::" in op_name else "vllm"
        op_part = op_name.split("::")[1] if "::" in op_name else op_name
        for lib in torch.library._get_libs():
            if lib.namespace == namespace:
                for fn in lib._ops.values():
                    if fn.name() == op_name:
                        return True
    except Exception:
        pass
...
def patch_direct_register_custom_op():
    """[h30014172] Monkey patch direct_register_custom_op to prevent duplicate registration.
    
    在多进程 Worker 启动时，vllm_ascend.patch.worker 模块会被多次导入，
    导致 direct_register_custom_op 被重复调用注册相同的 operator。
    此 patch 通过检查已注册的 operator 列表来防止重复注册。
    """
    global _DIRECT_REGISTER_CUSTOM_OP_PATCHED
    if _DIRECT_REGISTER_CUSTOM_OP_PATCHED:
        return
    
    from vllm.utils.torch_utils import direct_register_custom_op as original_direct_register_custom_op
    
    _registered_ops: dict[str, bool] = {}
    
    def patched_direct_register_custom_op(
        op_name: str,
        op_func,
        fake_impl=None,
        mutates_args=None,
    ...
    vllm.utils.torch_utils.direct_register_custom_op = patched_direct_register_custom_op
    
    import vllm.model_executor.layers.utils
    vllm.model_executor.layers.utils.direct_register_custom_op = patched_direct_register_custom_op
    
    _DIRECT_REGISTER_CUSTOM_OP_PATCHED = True


def adapt_patch(is_global_patch: bool = False):
    patch_direct_register_custom_op()
    if is_global_patch:
...
def asymmetric_divide(total: int, world_size: int, rank: int) -> tuple[int, int]:
    """[h30014172]Uneven split: remainder goes to the last rank.
    Args:
        total: Total size to split.
        world_size: Number of partitions.
        rank: Rank of the current partition.

    Returns:
        (local_size, local_offset) for the given rank.

    Examples:
        >>> asymmetric_divide(32, 3, 0)
        (10, 0)
        >>> asymmetric_divide(32, 3, 1)
        (10, 10)
        >>> asymmetric_divide(32, 3, 2)
        (12, 20)
        >>> asymmetric_divide(151936, 3, 0)
        (50645, 0)
        >>> asymmetric_divide(151936, 3, 2)
        (50646, 101290)
    """
    ...
    if rank < world_size - 1:
        local_size = base
        local_offset = rank * base
    else:
        local_size = base + remainder
        local_offset = (world_size - 1) * base
