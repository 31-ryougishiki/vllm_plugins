...
from .patch_qwen3_next import (
    ...
    QwenGatedDeltaNetAttentionAsymmetric
)
...
#
#             conv_state_shape = self.cls._orient_conv_shape(
#                 conv_state_dim,
...
#
#             conv_state_shape = self.cls._orient_conv_shape(#不知道是不是self.cls
...
class Qwen3_5DecoderLayerAsymmetric(OrigQwen3_5DecoderLayer):
    ...
    def __init__(
        ...
        # [0.23.0] Use hf_text_config instead of hf_config
        # linear_num_key_heads and other linear attention params are on text config
        ...
        # [0.23.0] config is already Qwen3_5TextConfig (from hf_text_config) and
        # has no .text_config; fall back to config itself for compatibility.
...
class Qwen3_5ForCausalLMAsymmetric(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__()
        # [0.23.0] hf_text_config already returns Qwen3_5TextConfig; the
        # 0.18.0-era .get_text_config() unwrap is redundant (returns self).
        config = vllm_config.model_config.hf_text_config
        quant_config = vllm_config.quant_config
