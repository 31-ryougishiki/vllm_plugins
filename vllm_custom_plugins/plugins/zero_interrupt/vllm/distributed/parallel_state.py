...
class GroupCoordinator:
    ...
    # [merge-0829] 支持 dynamic 长度的 all_gather，供 0829 变长
    # vocab/logits 路径使用。
...
# [merge-0829] kept for backward compatibility with the 0829 branch.
...
# =============================================================================
# [merge-0829] 0829 legacy asymmetric-TP helpers. These are used only by the
# non-DeepSeek-V4 zero-interrupt path (VLLM_ITS_DEEPSEEK_V4 unset/0).
# =============================================================================
def get_engine_parallel_config(zero_interrupt_config):
    ...
    for conf in zero_interrupt_config["engine_parallel_config"]:
        if str(executor_id) == str(conf.get("executor_id")):
            return conf
    raise ValueError(
        f"[lqf] fail to get engine_parallel_config for "
        f"executor_id={executor_id}, zero_interrupt_config={zero_interrupt_config}"
    )
...
def asym_world_size(zero_interrupt_config):
    world_size = 0
    for conf in zero_interrupt_config["engine_parallel_config"]:
        # 每个 dp-group 使用 new_tp 张 NPU。
        world_size += int(conf["new_tp"])
    assert world_size > 0, (
        f"[lqf] invalid zero_interrupt_config={zero_interrupt_config}"
    )
    return world_size


def get_global_rank_asym(rank):
    """按 data_parallel_rank 排序，累计前置 DP 的 new_tp 得到全局 rank。"""
    from vllm.config import get_current_vllm_config_or_none

    config = get_current_vllm_config_or_none()
    logger.debug(
        "[zero-interrupt-asym-mode] get_global_rank_asym, "
        "config is None=%s",
        config is None,
    )
    assert config is not None, "current VllmConfig is not available"
    ...
    sorted_engine_parallel_conf = sorted(
        engine_parallel_conf, key=lambda c: c["data_parallel_rank"]
    )
    offset = 0
    for c in sorted_engine_parallel_conf:
        if str(c["executor_id"]) == str(cur_executor_id):
            return offset + int(rank)
        offset += int(c["new_tp"])
    raise ValueError(
        f"fail in get_global_rank_asym, "
        f"zero_interrupt_config={zero_interrupt_config}"
    )
...
def init_distributed_environment_asym(
    ...
    """用于 0829 零中断缩容场景的非对称通信组重建。

    与 ``init_distributed_environment`` 的差别：全局 rank/world_size 由
    ``zero_interrupt_config.engine_parallel_config`` 累加得到，而不是
    ``data_parallel_rank * world_size`` 的均匀公式。elastic-EP 与旧
    stateless API 不兼容，显式禁用。
    """
    logger.info(
        "[zero-interrupt-asym-mode] world_size=%d rank=%d local_rank=%d "
    ...
    config = get_current_vllm_config_or_none()
    enable_elastic_ep = (
        config is not None and config.parallel_config.enable_elastic_ep
    )
    if enable_elastic_ep:
        raise NotImplementedError(
            "0829 asymmetric restart does not support elastic EP"
        )
    ...
    if (
        ...
        if parallel_config.nnodes > 1:
            ...
            distributed_init_method = get_distributed_init_method(ip, port)
            logger.debug(
                "Adjusting world_size=%d rank=%d distributed_init_method=%s "
                "for DP",
                world_size,
                rank,
                distributed_init_method,
    ...
    if not torch.distributed.is_initialized():
        ...
        if not torch.distributed.is_backend_available(backend):
            logger.warning(
                "Distributed backend %s is not available; falling back "
                "to gloo.",
                backend,
            )
    ...
    if _WORLD is None:
        ...
        if config is not None and parallel_config.nnodes > 1:
            _NODE_COUNT = parallel_config.nnodes
        else:
            _NODE_COUNT = _node_count(_WORLD.cpu_group)
        logger.info(
            "[zero-interrupt-asym-mode] Detected %d nodes in the distributed "
            "environment",
            _NODE_COUNT,
        )
    else:
    ...
    if (
        ...
        group_ranks = [
            [dp_rank * world_size_inner_dp + i
             for i in range(world_size_inner_dp)]
...
def initialize_model_parallel(
    ...
    if asym:
        # [merge-0829] 0829 旧非对称路径：TP groups 是每 DP 连续
        # new_tp 个 rank；DP groups 按位置 zip；DCP/PCP/PP 均为 singleton；
        # MoE EP group 覆盖全部 rank。与 DeepSeek-V4 hetero 分支互斥。
        ...
        if additional_config is None:
            raise ValueError(
                "initialize_model_parallel(asym=True) requires "
                "additional_config.zero_interrupt_config"
            )
        zero_interrupt_config = additional_config.get(
            "zero_interrupt_config", None
        )
        if zero_interrupt_config is None:
            raise ValueError(
                "initialize_model_parallel(asym=True) requires "
                "zero_interrupt_config"
            )
        sorted_engine_parallel_conf = sorted(
            zero_interrupt_config["engine_parallel_config"],
            key=lambda c: c["data_parallel_rank"],
        )
        ...
        tp_groups = []
        next_rank = 0
        for conf in sorted_engine_parallel_conf:
            new_tp = int(conf["new_tp"])
            if new_tp <= 0:
                continue
            tp_groups.append(
                [r for r in range(next_rank, next_rank + new_tp)]
            )
            next_rank += new_tp
        total_ranks = next_rank
        ...
        dp_groups = []
        for ranks in zip_longest(*tp_groups):
            filtered = [r for r in ranks if r is not None]
            if filtered:
                dp_groups.append(filtered)
        ...
        singleton_groups = [[r] for r in range(total_ranks)]
...
def patch_tensor_parallel_group(tp_group: GroupCoordinator):
    """[merge-0829] 临时替换 TP group，用于 draft model 与 target model
    TP degree 不同的 speculative decoding 场景。"""
