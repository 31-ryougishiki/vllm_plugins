...
def _is_deepseek_v4_kv_mode() -> bool:
    """Read the same runtime switch as the zero_interrupt dispatcher.

    This file is installed as ``vllm/v1/core/kv_cache_utils.py`` and is used
    before/around the plugin monkey-patches, so it cannot import the plugin
    package. Reading the environment at call time keeps the installed file
    usable in both the DeepSeek-V4 and 0829 runtime families.
    """
...
def get_kv_cache_config_from_groups(
    ...
    if len(kv_cache_groups) == 1 and isinstance(
        ...
        if len(page_sizes) > 1:
            # [merge-0829] B 路径的混合 page_size 分配（例如 DEGRADE
            # TP=4->3 时 FullAttention + Mamba 的 page_size 不一致）。
            # DeepSeek-V4 的 all-UniformTypeKVCacheSpecs 分支在上面已返回。
            ...
            for group in kv_cache_groups:
                ...
                tensor_size = ps * num_blocks
                for layer_name in group.layer_names:
                    kv_cache_tensors.append(
                        KVCacheTensor(
                            size=tensor_size, shared_by=[layer_name]
                        )
                    )
            return KVCacheConfig(
...
def get_kv_cache_groups(
    ...
    # will raise an error.
    # [merge-0829] DeepSeek-V4 保留 v0.23 的 page-size 统一行为；0829 旧
    # 非对称路径保留不同 page_size，由 get_kv_cache_config_from_groups 的
    # mixed-page_size 分支处理。
    if _is_deepseek_v4_kv_mode():
...
def generate_scheduler_kv_cache_config(
    ...
    """
    # [merge-0829] B 路径可能因混合 page_size 得到不同的 num_blocks；
    # scheduler 必须取最小值，避免 over-allocate。
    num_blocks_values = [cfg.num_blocks for cfg in kv_cache_configs]
    if len(set(num_blocks_values)) > 1:
        min_num_blocks = min(num_blocks_values)
...
def _max_memory_usage_bytes_from_groups(
    ...
    # [merge-0829] B 路径：混合 page_size（FullAttention + Mamba 等）按
    # group 分别计算后求和；统一 page_size 时行为与 v0.23 原逻辑一致。
    page_sizes = {
        group.kv_cache_spec.page_size_bytes for group in kv_cache_groups
    }
    if len(page_sizes) > 1:
        ...
        for group in kv_cache_groups:
            spec = group.kv_cache_spec
            g_page_size = spec.page_size_bytes
            g_blocks_needed = cdiv(
                spec.max_memory_usage_bytes(vllm_config), g_page_size
            )
            g_memory = len(group.layer_names) * g_page_size * g_blocks_needed
...
def project_kv_cache_groups_to_worker_asym(
    global_kv_cache_groups: list[KVCacheGroupSpec],
    worker_spec: dict[str, KVCacheSpec],
) -> list[KVCacheGroupSpec]:
    """[merge-0829] 0829 非对称路径的 worker 投影。

    与 v0.23 原实现的关键差异：当 worker 实际 spec 与全局 group spec 的
    page_size_bytes / shapes 不一致时（非对称 TP 重分片后常见），以
    worker_spec 为准，避免按错误尺寸分配 KV cache。
    """
    projected_groups: list[KVCacheGroupSpec] = []
    for group in global_kv_cache_groups:
        worker_layer_names = [
        ...
        group_spec = group.kv_cache_spec
        old_group_spec = worker_spec[worker_layer_names[0]]

        page_size_attrs = (
            hasattr(group_spec, "page_size_bytes")
            and hasattr(old_group_spec, "page_size_bytes")
        )
        if page_size_attrs:
            page_size_match = (
                group_spec.page_size_bytes == old_group_spec.page_size_bytes
            )
            if page_size_match:
                shapes_match = (
                    not hasattr(group_spec, "shapes")
                    or not hasattr(old_group_spec, "shapes")
                    or group_spec.shapes == old_group_spec.shapes
                )
                chosen_spec = group_spec if shapes_match else old_group_spec
            else:
                chosen_spec = old_group_spec
        elif hasattr(group_spec, "shapes") and hasattr(
            old_group_spec, "shapes"
        ):
            shapes_match = group_spec.shapes == old_group_spec.shapes
            chosen_spec = group_spec if shapes_match else old_group_spec
        else:
            chosen_spec = group_spec
...
def get_kv_cache_configs(
    ...
    deepseek_v4_mode = _is_deepseek_v4_kv_mode()
    for kv_cache_spec_one_worker in kv_cache_specs:
        for layer_name, layer_spec in kv_cache_spec_one_worker.items():
            if layer_name not in merged_kv_cache_specs:
                merged_kv_cache_specs[layer_name] = layer_spec
            elif deepseek_v4_mode:
                assert merged_kv_cache_specs[layer_name] == layer_spec, (
                    "The KV cache specs for the same layer are different "
                    "across workers. This is not supported yet."
                )
            else:
                # [merge-0829] 0829 非对称路径允许同层 spec 跨 worker
                # 不同（TP 分片后 page_size/shapes 变化），取更大
                # page_size 以覆盖所有 worker。
                existing = merged_kv_cache_specs[layer_name]
                if layer_spec.page_size_bytes > existing.page_size_bytes:
                    merged_kv_cache_specs[layer_name] = layer_spec
    ...
    # [merge-0829] DeepSeek-V4 使用 v0.23 原投影；0829 非对称使用
    # worker-spec 优先投影。
    projector = (
        _project_kv_cache_groups_to_worker
        if deepseek_v4_mode
        else project_kv_cache_groups_to_worker_asym
    )
    projected_groups_per_worker = [
        projector(global_kv_cache_groups, worker_spec)
    ...
    if not deepseek_v4_mode:
        # [merge-0829] 0829 混合 page_size 场景直接返回，不再做跨
        # worker num_blocks 归一化（各 bucket 独立分配）。
        has_mixed_page_sizes = any(
            len(cfg.kv_cache_tensors) != len(cfg.kv_cache_groups)
            for cfg in kv_cache_configs
        )
        if has_mixed_page_sizes:
            for cfg in kv_cache_configs:
                if len(cfg.kv_cache_groups) > 0:
                    _report_kv_cache_config(vllm_config, cfg)
    ...
    for kv_cache_config in kv_cache_configs:
        ...
        if not deepseek_v4_mode:
            # [merge-0829] 0829 路径按 layer 的实际 page_size 计算
            # 原始 num_blocks，再按 min_num_blocks 收缩。
            layer_to_page_size = {}
            for group in kv_cache_config.kv_cache_groups:
                ps = group.kv_cache_spec.page_size_bytes
                for layer_name in group.layer_names:
                    layer_to_page_size[layer_name] = ps
            for tensor in kv_cache_config.kv_cache_tensors:
                if (
                    tensor.shared_by
                    and tensor.shared_by[0] in layer_to_page_size
                ):
                    ps = layer_to_page_size[tensor.shared_by[0]]
                    original_num_blocks = tensor.size // ps
                else:
                    original_num_blocks = num_blocks_old
                    ps = (
            ...
            # DeepSeek-V4 / v0.23 原逻辑：tensor.size 与 num_blocks 等比。
            for tensor in kv_cache_config.kv_cache_tensors:
